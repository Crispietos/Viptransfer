  ...
  useEffect(() => {
    loadGoogleMapsScript(() => {
      setTimeout(() => {
        ...
        if (input1) {
          console.log("📍 Inicjalizacja Autocomplete dla pickupLocation");
          try {
        ...
        if (input2) {
          console.log("📍 Inicjalizacja Autocomplete dla dropoffLocation");
          try {
      ...
      <div className="box-tab-left">
        <div className="box-content-detail">
          ...
          <div className="col-lg-12 text-center mt-20">
            <button type="button" className="btn btn-primary" onClick={calculateDistance}>Calculate Price</button>
          </div>

          {distance && (
            <div className="col-lg-12 text-center mt-4">
              <p className="text-24 font-bold mt-3">Distance: {distance.toFixed(1)} km</p>
              <p className="text-32 font-bold mt-2" style={{ padding: "20px", border: "2px solid #000", display: "inline-block" }}>Estimated Price: €{price}</p>
            </div>
          )}
        </div>
      </div>
